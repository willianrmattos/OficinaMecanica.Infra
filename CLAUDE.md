...
Raiz (`*.tf`) + 12 modulos (pasta propria com `main.tf`/`variables.tf`/`outputs.tf` cada):
...
- **`helm/`**: `ingress-nginx` (chart oficial, namespace proprio, Service `LoadBalancer`, com a annotation `azure-load-balancer-health-probe-request-path: /healthz` — sem ela, o health probe do proprio Load Balancer do Azure bate em `GET /`, cai na regra catch-all da API e recebe `301` da Swagger UI em vez de `200`, fazendo o Azure bloquear **todo** trafego externo por considerar o `ingress-nginx` inteiro unhealthy; tambem seta `controller.config.use-forwarded-headers: true` — sem isso o ingress-nginx **ignora** os `X-Forwarded-Proto`/`-Host`/`-Prefix` que a policy da APIM injeta e os substitui pelos proprios valores computados, ver `apim/` abaixo), `otel-collector` (`open-telemetry/opentelemetry-collector`, `mode: deployment`, 1 replica — recebe traces/metricas/logs OTLP da API e reexporta pra New Relic via exporter `otlphttp` generico, `headers.api-key` = license key) e `newrelic` (chart `nri-bundle` — integracao de Kubernetes da New Relic, `kube-state-metrics` proprio ja que nao ha mais nenhum concorrente no cluster). Prometheus/Grafana/Loki/Alloy/Jaeger (kube-prometheus-stack + Loki/Alloy self-hosted) foram removidos do cluster e substituidos pelo New Relic (decisao explicita: escolha livre de ferramenta de observabilidade pro projeto). Usa o provider `helm` configurado em `providers.tf` apontando pro `aks` via kube_config. Namespace `monitoring` recriado como `kubernetes_namespace` explicito (nao mais `create_namespace=true` de um helm_release especifico) - o Secret nativo com a license key da New Relic (`kubernetes_secret.newrelic_license_key`, unico consumidor de `azurerm_key_vault_secret.newrelic_license_key` na raiz) precisa que o namespace ja exista antes dele
- **`github_oidc/`**: App Registration + Service Principal + Federated Identity Credential (OIDC, restrita a `repo:<owner>/<repo>:ref:refs/heads/main` e `ref:refs/heads/release`) usados pelo GitHub Actions do repo `OficinaMecanica` pra autenticar no Azure sem nenhum secret de longa duracao. Role assignments `AcrPush` (no `acrfiap`), `Azure Kubernetes Service Cluster Admin Role` (no `aksfiap`) e `Key Vault Secrets User` (no `kvfiap`, pro step de migration ler a connection string) — ver secao "CI/CD" no repo `OficinaMecanica`
- **`github_oidc_infra/`**: identidade OIDC unica do proprio repositorio (`OficinaMecanica.Infra`) - cobre tanto `terraform plan` (PR) quanto `terraform apply` de verdade (push em `main`/`release`). `Contributor` no resource group inteiro (cobre tudo que este repo gerencia), `Storage Blob Data Contributor` no container do tfstate (RBAC de dados, separado do Contributor de control-plane - sem ele o apply le o state mas nao consegue escrever de volta), `Key Vault Administrator` (esse repo cria/gerencia secrets, nao so le) e `Application.ReadWrite.All` (Microsoft Graph, com admin consent) - necessario porque os modulos `github_oidc*` deste repo gerenciam suas proprias App Registrations via Terraform
- **`github_oidc_banco/`**: mesmo padrao de identidade unica, mas pro repositorio irmao `OficinaMecanica.Banco` - `Reader` no resource group (so pra resolver o `data "azurerm_resource_group" "existing"` de la), `Storage Blob Data Contributor` no tfstate, e `Contributor` escopado especificamente no SQL Server (`svsfiap`, unico recurso que aquele repo de fato gerencia)
...
- **`github_oidc_seguranca/`**: mesmo padrao OIDC do `github_oidc/` acima, modulo **separado** (nao uma segunda instancia do `github_oidc`) porque a permissao necessaria e bem diferente — restrito ao repositorio `OficinaMecanica.Seguranca`, role assignment `Contributor` escopado so na propria Function App (`module.functionapp.function_app_id`, suficiente pra deploy via zip), sem AcrPush nem nenhuma role no cluster AKS. Tambem tem `Key Vault Secrets User` (no `kvfiap`) pro step de migration daquele repo ler a connection string
- **`apim/`**: Azure API Management (`apimfiap`), SKU `Consumption_0` (unico valor aceito nesse tier - serverless, sem capacidade dedicada, sempre gratis ate 1M chamadas/mes). Fica **na frente** do `ingress-nginx` (nao o substitui) - o `ingress-nginx` continua servindo Mailpit diretamente e vira so o *backend* que a APIM chama. Modelo **wildcard/passthrough** (nao import de OpenAPI): cada API (`oficinamecanica-api` no path `oficinaserver`, `seguranca-api` no path `segurancaserver`) e criada "em branco" com uma `azurerm_api_management_api_operation` coringa (`url_template = "/*"`) por metodo HTTP real via `for_each = toset(["GET", "POST", "PUT", "DELETE", "PATCH"])` — a APIM **nao tem um metodo HTTP curinga de verdade**, `method = "*"` e aceito pelo schema do provider Terraform sem erro mas o runtime da Azure nunca casa com nada (404 silencioso pra qualquer request). Backends definidos como entidades nomeadas e reutilizaveis (`azurerm_api_management_backend`: `ingress_nginx` e `seguranca`), referenciadas via `<set-backend-service backend-id="...">` na policy de cada API (`azurerm_api_management_api_policy`) — nao `service_url` inline. O backend `seguranca` e diferente do outro: aponta direto pro hostname publico da Function App (`module.functionapp.default_hostname`, `<nome>.azurewebsites.net`), sem passar pelo `ingress-nginx`/AKS. O IP do LoadBalancer do `ingress-nginx` e lido dinamicamente via `data "kubernetes_service"` (provider `kubernetes`, configurado em `providers.tf` reaproveitando os mesmos outputs de `module.aks` que o provider `helm` ja usa). `subscription_required = false` nas duas (a API ja tem seu proprio JWT + rotas publicas de proposito; Seguranca emite os JWT, nao valida nenhum aqui). Nomenclatura de path pensada pra crescer: cada backend novo entra como `<nome>server` (ex.: `segurancaserver`), mesmo padrao ja usado por `oficinaserver`. A API `grafana` (path `/grafana`, backend/operations/policy/product_api proprios) existiu aqui e foi removida junto com o Grafana do cluster (ver bullet `helm/` acima) — o Grafana e o resto do stack antigo foram substituidos pelo New Relic, cujo acesso e direto (SaaS, fora da APIM).

  A policy da API `oficinamecanica` injeta os headers de proxy que o backend precisa pra saber que esta atras de um gateway publico: `X-Forwarded-Proto: https`, `X-Forwarded-Host: <apim>.azure-api.net` e `X-Forwarded-Prefix: /oficinaserver`. `ingress-nginx` precisa de `use-forwarded-headers: true` (`helm/`, acima) pra nao descartar esses headers e preenche-los sozinho — esse foi o bug raiz por tras do `servers[]` do Swagger aparecendo com o IP interno em vez do host publico da APIM. O lado aplicacao (`Program.cs` do repo `OficinaMecanica`) le esses headers via `ForwardedHeadersMiddleware` + middleware customizado pra `X-Forwarded-Prefix`. A policy do `seguranca` e diferente: usa `X-Gateway-Proto`/`X-Gateway-Host`/`X-Gateway-Prefix` (nomes proprios, NAO `X-Forwarded-*`) porque o App Service intercepta e descarta silenciosamente qualquer header client-supplied comecando com `X-Forwarded-` antes de chegar no worker isolado (confirmado na pratica) — o `ServerBasePathDocumentFilter.cs` (repo `OficinaMecanica.Seguranca`) le esses headers `X-Gateway-*` pra montar o `servers[]` do OpenAPI e o redirect da raiz com a URL publica certa da APIM.
...
Comandos do Terraform sempre a partir da raiz deste repositorio como
working directory, tanto rodando manualmente quanto via CI/CD.

## CI/CD

`.github/workflows/ci.yml` automatiza `plan`/`apply` desta propria infra via
GitHub OIDC (`github_oidc_infra/`, identidade unica cobrindo os dois): job
`validate` (`fmt -check` + `validate` + `plan -lock=false`) roda em todo
PR/push contra `main`/`release`; job `apply` (`terraform apply -auto-approve`)
so em push/disparo manual nessas branches. O deploy da *aplicacao* (nao da
infra) e automatizado nos repositorios `OficinaMecanica`/
`OficinaMecanica.Seguranca`, ver o `CLAUDE.md` de cada um, secao "CI/CD".
...
Isso e pra rodar manualmente/localmente - ver secao "CI/CD" acima pra como
isso ja roda automaticamente.
