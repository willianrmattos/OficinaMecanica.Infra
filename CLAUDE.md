...
Raiz (`*.tf`) + 11 modulos (pasta propria com `main.tf`/`variables.tf`/`outputs.tf` cada):
...
- **`sqldb/`**: Azure SQL Database (`svsfiap.database.windows.net` / `OficinaMecanicaDb`), serverless, tier sempre-gratis. O campo que ativa esse tier (`use_free_limit`) nao existe no provider `azurerm` e nao pode ser setado depois via `az sql db update` (so na criacao) — por isso o banco foi criado via `az sql db create --use-free-limit true --free-limit-exhaustion-behavior AutoPause ...` e depois trazido para o state do Terraform com `terraform import`. Segundo banco (`azurerm_mssql_database.seguranca`, database `SegurancaDb`) no **mesmo** servidor logico `svsfiap` (nao um servidor novo) pro `OficinaMecanica.Seguranca` — serverless comum (sem o tier sempre-gratis, que so vale 1x por assinatura e ja foi usado pelo banco principal), com `auto_pause_delay_in_minutes` pra manter o custo minimo
